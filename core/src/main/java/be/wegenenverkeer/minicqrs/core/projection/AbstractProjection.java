...
public abstract class AbstractProjection<ID, E> {
  ...
  @Autowired
  protected TransactionalOperator rxtx;

  @Autowired
  protected DSLContext ctx;

  public AbstractProjection(Cache<ProjectionId, Long> cache, Set<Long> shards, Class<E> eventClass) {
  ...
  private Mono<Map<Long, Optional<Long>>> getOffsetsFromMemory() {
    return Mono.fromCallable(() -> shards.stream()
        .map(shard -> Pair.of(shard, Optional.ofNullable(cache.get(new ProjectionId(getProjectionName(), shard)))))
  ...
  private Mono<Map<Long, Optional<Long>>> getOffsetsFromRepo(Map<Long, Optional<Long>> maybeMemoryOffsets) {
    ...
    return projectionOffsetRepository.getOffsets(getProjectionName(), shardsToGet)
        .collectList()
        .map(r -> {
  ...
  private Mono<Void> upsertOffsetWithCheck(String projection, long shard, long sequence) {
    return projectionOffsetRepository.upsertOffset(projection, shard, sequence)
        .handle((r, sink) -> {
          if (r == 0)
  ...
  private Mono<List<EventHolder<E>>> getEventsSince(Map<Long, Optional<Long>> offsets) {
    return Flux.mergeSequential(offsets.entrySet().stream().map(e -> {
      ...
      return journalRepository.getEventsOnShardSince(shard, since, getMaxEvents(), eventType).collectList();
  ...
  private Mono<Map<Long, Long>> applyEvents(List<EventHolder<E>> events) {
    Map<Long, Long> offsets = events.stream()
        .collect(Collectors.groupingBy(EventHolder::shard,
