...
public abstract class AbstractGroupedByIdProjection<ID, E> extends AbstractProjection<ID, E> {
  public AbstractGroupedByIdProjection(
  ...
  protected Mono<Void> handleEvents(List<EventHolder<E>> events) {
    Map<String, List<EventHolder<E>>> grouped =
        events.stream().collect(Collectors.groupingBy(e -> e.id()));
    return Flux.concat(
            grouped.entrySet().stream()
                .map(
                    e ->
                        handleEventsById(
                            toId(e.getKey()), e.getValue().stream().map(r -> r.event()).toList()))
                .toList())
        .collectList()
        .then();
  }
