...
public class SnapshotRepository<S> {
  ...
  private SnapshotRecord toDb(StateHolder<S> stateHolder) {
    try {
      return new SnapshotRecord(
          stateHolder.id(),
          jsonb(objectMapper.writeValueAsString(stateHolder.state())),
          stateHolder.sequence(),
          stateHolder.type().toCanonical());
  ...
  private StateHolder<S> fromDb(SnapshotRecord record) {
    try {
      ...
      return new StateHolder<S>(
          record.getId(),
          objectMapper.readValue(record.getSnapshot().data(), type),
          record.getSequence(),
          type);
    } catch (JsonProcessingException e) {
  ...
  public Mono<Optional<StateHolder<S>>> getLatestSnapshot(String id, JavaType stateType) {
    ...
    return Mono.from(
            ctx.selectFrom(SNAPSHOT)
                .where(and(SNAPSHOT.ID.eq(id), SNAPSHOT.TYPE.eq(stateType.toCanonical())))
                .limit(1))
